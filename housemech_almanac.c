 ...
 *
 *
 * housemech_almanac.c - Interface with the almanac services.
 *
 * SYNOPSYS:
 *
 ...
 *    the last 24 hours.
 *    The purpose is to delay rules execution until at least one almanac
...
const time_t housemech_almanac_sunset (void) {
    return SunSet;
}

const time_t housemech_almanac_sunrise (void) {
    return SunRise;
}

static ParserToken *housemech_almanac_prepare (int count) {
...
static void housemech_almanac_update (const char *provider,
   ...
   ParserToken *tokens = housemech_almanac_prepare (count);
...
static void housemech_almanac_discovered
   ...
   if (!status) {
       echttp_submit (0, 0, housemech_almanac_discovered, origin);
       return;
   }
   ...
   housemech_almanac_update (provider, data, length);
}

static void housemech_almanac_scan_server
    ...
    echttp_submit (0, 0, housemech_almanac_discovered, (void *)provider);
}
...
void housemech_almanac_background (time_t now) {
    ...
    housediscovered ("almanac", 0, housemech_almanac_scan_server);
}
